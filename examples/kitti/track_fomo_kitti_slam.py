...
size = Image.open(os.path.join(sequence_path, 'image_0', '000000.png')).size
...
# Prepare unified frames metadata (Cameras only)
frames_metadata = []
...
for i, t_ns in enumerate(timestamps):
    frames_metadata.append({
        ...
        'timestamp': t_ns,
        'images_paths': [
            os.path.join(sequence_path, f'image_{cam}', f'{i:0>6}.png')
            for cam in [0, 1]
        ]
    })
