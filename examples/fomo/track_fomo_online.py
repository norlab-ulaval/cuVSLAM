...
start_data_time_ns = None
skip_threshold_ns = 10_000_000 # 10 ms tolerance
...
for metadata in frames_metadata:
    ...
    # Initialize clocks on the first processed frame
    if start_real_time is None:
        start_real_time = time.perf_counter()
        start_data_time_ns = timestamp

    # Calculate expected timestamp based on elapsed real-world time
    elapsed_real_s = time.perf_counter() - start_real_time
    expected_timestamp_ns = start_data_time_ns + int(elapsed_real_s * 1e9)

    # If we are ahead of real-time, sleep to maintain exact real-time playback
    if timestamp > expected_timestamp_ns:
        sleep_s = (timestamp - expected_timestamp_ns) / 1e9
        time.sleep(sleep_s)
        # Update expected timestamp after sleeping
        expected_timestamp_ns = timestamp
    ...
    # If we are lagging behind the real-time clock, skip tracking this image
    if timestamp < expected_timestamp_ns - skip_threshold_ns:
        skipped_frames_count += 1
        frame_idx += 1
        print(f"[Warning] Lagging behind. Skipping camera frame {frame_idx}")
        continue
...
print(f"Number of skipped frames: {skipped_frames_count}")
